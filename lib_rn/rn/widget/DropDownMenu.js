...
export default class TopMenu extends Component {
    ...
    constructor(props) {
        ...
        //     const source = {selectedIndex: 0, type: 'title'};
        //     Object.assign(data, source);
        //     let arrAdd = [{"id": "0", "name": "全部"}];
        //     let arr = data[i].accessControl;
        //     data[i].accessControl = [...arrAdd, ...arr]
        // }
        for (let i = 0, c = array.length; i < c; ++i) {

            const source = {selectedIndex: 0, type: 'title'};
            Object.assign(array, source);
            let arrAdd = [{"id": "null", "name": "全部"}];
            let arr = array[i].accessControl;
            array[i].accessControl = [...arrAdd, ...arr]

            let item = array[i];
            // top[i] = item.data[item.selectedIndex].title;
            top[i] = item.name;
            maxHeight[i] = Math.min(item.accessControl.length, max) * 43;
    ...
    hide = (index, subselected) => {
        ...
        if (subselected !== undefined) {
            ...
            // this.state.top[index] = this.props.config[index].accessControl[subselected].name;
        ...
        return (
            <Animated.View key={index} pointerEvents={enabled ? 'auto' : 'none'}
                ...
                <ScrollView style={styles.scroll}>
                    {d.accessControl.map((data, subindex) => {
                        return <Comp
    ...
    render() {
        ...
        if (this.state.selectedIndex !== null) {
            // list = this.props.config[this.state.selectedIndex].accessControl;
            list = this.state.array[this.state.selectedIndex].accessControl;
        }
        console.log(list);
        return (
            <View style={{flex: 1}}>
